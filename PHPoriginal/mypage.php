  ...
  <div id="wrapper">
   ...
   <div id="all">
    <div class="mypage">
      ...
      <p><?php if(isset($date['create_at'])): ?>
        <?php date_default_timezone_set('Asia/Tokyo');
              $day = new DateTime($date['create_at']);
              $today = new DateTime('now');
              $diff = $day->diff($today);
              echo $diff->format('禁煙を開始して%a日目です。');?></p>

       <!--  <p><?php $num = $diff->format('%a');
              $sum = $num*550;
              echo "1日1箱吸っていた場合".$sum."円節約できています。";?></p>
        <p>*1箱550円で計算</p> -->
        
      <?php else: ?>
        <p>禁煙掲示板で禁煙宣言をするとその日から禁煙日数が表示されます。</p>
      <?php endif; ?>
      </div>
